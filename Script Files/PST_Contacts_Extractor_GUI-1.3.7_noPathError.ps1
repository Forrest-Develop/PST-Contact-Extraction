...
function Show-PstContactsFolderPicker([string]$pstPath) {
  ...
  # Find first Contacts node to preselect (if present)
  function Get-FirstContactsNode([System.Windows.Forms.TreeNode]$node) {
    if ($node.NodeFont -and $node.NodeFont.Style.HasFlag([System.Drawing.FontStyle]::Bold)) { return $node }
    foreach ($child in $node.Nodes) {
      $hit = Get-FirstContactsNode $child
      if ($hit) { return $hit }
    }
    return $null
  }

  $firstContacts = Get-FirstContactsNode $rootNode
  $tree.SelectedNode = if ($firstContacts) { $firstContacts } else { $rootNode }
  $ok.Enabled = $true

  # Holder to avoid event handler scope issues
  $pick = New-Object psobject -Property @{ Value = $null }

  # Always select the node on single-click
  $tree.Add_NodeMouseClick({ param($s,$e)
      if ($e.Node) {
  ...
  # Accept double-click
  $tree.Add_NodeMouseDoubleClick({ param($s,$e)
      if ($e.Node -and $e.Node.Tag) {
        $tree.SelectedNode = $e.Node
        $pick.Value = $e.Node.Tag
        $form.Close()
      }
  })

  # Keep OK enabled in sync with selection
  $tree.Add_AfterSelect({ param($s,$e)
      $ok.Enabled = ($e.Node -ne $null -and $e.Node.Tag -ne $null)
  })

  $ok.Add_Click({
    if ($tree.SelectedNode -and $tree.SelectedNode.Tag) {
      $pick.Value = $tree.SelectedNode.Tag
      $form.Close()
    } else {
  ...
  $form.Add_Shown({ $Global:SuppressPopups = $false })
...
# - Reinstated manual folder verification checkbox in the main form
# -------------------------------------------------------------
...
"@
    ...
    try {
        # Find a base directory that works in both .ps1 and ps2exe .exe
        $baseDir = $null
        if ($PSScriptRoot) { $baseDir = $PSScriptRoot }
        elseif ($PSCommandPath) { $baseDir = Split-Path -Parent $PSCommandPath }
        elseif ([System.AppContext]::BaseDirectory) { $baseDir = [System.AppContext]::BaseDirectory }

        if (-not [string]::IsNullOrWhiteSpace($baseDir)) {
            $licenseFile = Join-Path $baseDir 'LICENSE.txt'
...
# Gate execution unless bypassed for trusted automation (minimal, PS5-safe)
# Resolve LOCALAPPDATA reliably even if the env var is missing
$localApp = $env:LOCALAPPDATA
if ([string]::IsNullOrWhiteSpace($localApp)) {
    try { $localApp = [Environment]::GetFolderPath('LocalApplicationData') } catch { $localApp = $null }
}
if ([string]::IsNullOrWhiteSpace($localApp) -and $HOME) {
    $localApp = Join-Path $HOME 'AppData\Local'
}

$licenseMarker = if (-not [string]::IsNullOrWhiteSpace($localApp)) {
    Join-Path $localApp 'PSTContactsExtractor\license.accepted'
} else { $null }

if (-not $AcceptLicense) {
    if ($licenseMarker -and (Test-Path -LiteralPath $licenseMarker)) { $AcceptLicense = $true }
}
if (-not $AcceptLicense) {
...
try {
    if ($licenseMarker) {
        $dir = Split-Path -Parent $licenseMarker
        if ($dir -and -not (Test-Path -LiteralPath $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
        Set-Content -Path $licenseMarker -Value (Get-Date).ToString('o') -Encoding ASCII
    } else {
        Write-Log -Message 'Warning: could not determine LocalAppData; skipping license marker file.' -Color 'Gray'
    }
} catch { }
...
# --- General helpers (PS5/ps2exe-safe) ---
...
function Safe-ResolvePath {
    param([string]$p)
    if (Test-NonEmptyPath $p -and (Test-Path -LiteralPath $p)) {
        return (Resolve-Path -LiteralPath $p).Path
    }
    return $null
}

function Scroll-ControlToEnd {
    param($ctrl)
    if (-not $ctrl) { return }

    $t = $ctrl.GetType().FullName

    if ($t -in @('System.Windows.Forms.RichTextBox','System.Windows.Forms.TextBox','System.Windows.Forms.MaskedTextBox')) {
        try {
            if ($ctrl.PSObject.Properties['SelectionStart']) { $ctrl.SelectionStart = $ctrl.TextLength }
            if ($ctrl.PSObject.Properties['SelectionLength']) { $ctrl.SelectionLength = 0 }
            if ($ctrl.PSObject.Properties['ScrollToCaret']) { $ctrl.ScrollToCaret() }
        } catch { }
        return
    }

    if ($t -eq 'System.Windows.Forms.ListBox') {
        try { if ($ctrl.Items.Count -gt 0) { $ctrl.TopIndex = $ctrl.Items.Count - 1 } } catch { }
        return
    }
}
...
# Determine script/exe directory safely (PS5 + ps2exe)
$scriptDir = $null
...
elseif ([System.AppContext]::BaseDirectory) { $scriptDir = [System.AppContext]::BaseDirectory }
else { try { $scriptDir = (Get-Location).Path } catch { $scriptDir = '' } }

# --- Safe icon load (ps2exe + plain PS) ---
try {
    $basePath = $null
    if ($PSScriptRoot) { $basePath = $PSScriptRoot }
    elseif ($PSCommandPath) { $basePath = Split-Path -Parent $PSCommandPath }
    elseif ([System.AppContext]::BaseDirectory) { $basePath = [System.AppContext]::BaseDirectory }
    if (-not [string]::IsNullOrWhiteSpace($basePath)) {
        $icoPath = Join-Path $basePath 'app.ico'
        if (Test-Path -LiteralPath $icoPath) {
            $Form.Icon = [System.Drawing.Icon]::new($icoPath)
        } else {
            Write-Log -Message "app.ico not found at $icoPath (skipping window icon)" -Color 'Gray'
        }
    } else {
        Write-Log -Message "app base path empty; skipping window icon" -Color 'Gray'
    }
}
catch {
...
$chkManualVerify.Location = New-Object System.Drawing.Point(18, ($logBox.Bottom + 10))

$btnRun = New-Object System.Windows.Forms.Button
$btnRun.Text = 'Extract Contacts'
$btnRun.Width = 170
$btnRun.Enabled = $false
$btnRun.Location = New-Object System.Drawing.Point(18, ($chkManualVerify.Bottom + 12))
...
$btnOpenOut.Location = New-Object System.Drawing.Point(($btnRun.Right + 12), $btnRun.Top)

$btnClose = New-Object System.Windows.Forms.Button
$btnClose.Text = 'Close'
$btnClose.Location = New-Object System.Drawing.Point(592, $btnRun.Top)
$btnClose.Width = 88
...
if ($null -eq $folder) { throw "No folder selected." }
try { Write-Log -Message ("Selected folder: " + ($folder.Name)) -Color 'Gray' } catch {}

# If user picked a non-Contacts folder, gather any Contacts subfolders beneath it
if ($folder.DefaultItemType -ne 2) {
    $contactsFolders = New-Object System.Collections.Generic.List[object]
    function Get-ContactsSubfolders($f) {
        foreach ($sub in $f.Folders) {
            try {
                if ($sub.DefaultItemType -eq 2) { [void]$contactsFolders.Add($sub) }
            } catch {}
            Get-ContactsSubfolders $sub
        }
    }
    Get-ContactsSubfolders $folder

    if ($contactsFolders.Count -eq 0) {
        throw "The selected folder has no Contacts subfolders. Please pick a Contacts folder (bold) or a parent that contains one."
    }

    # Merge all contacts from those subfolders into a single CSV
    $olItemClassContact = 40
    $rows = New-Object System.Collections.Generic.List[object]
    foreach ($cf in $contactsFolders) {
        foreach ($item in @($cf.Items)) {
            try {
                if ($item -and $item.Class -eq $olItemClassContact) {
                    $rows.Add( (Convert-OutlookContactToCsvRow $item) )
                }
            } catch {}
        }
    ...
    $rows | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvPath
    Write-Log -Message "Manual export: $($rows.Count) contacts from $($contactsFolders.Count) folder(s) under '$($folder.Name)'" -Color 'SeaGreen'
} else {

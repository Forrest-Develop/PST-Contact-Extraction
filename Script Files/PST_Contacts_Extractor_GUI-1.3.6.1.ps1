...
$licenseAcceptedThisRun = $false

if (-not $AcceptLicense) {
    if (Test-Path $licenseMarker) {
        $AcceptLicense = $true
    } else {
...
if ($licenseAcceptedThisRun -and $AcceptLicense) {
    try {
        ...
        Write-Warning "Unable to persist license acceptance marker: $($_.Exception.Message)"
    }
}

...
# Load icon from the script directory, not the process working dir
$iconPath = Join-Path $scriptDir 'app.ico'
try {
    if (Test-Path $iconPath) {
        # Load a .ico file directly
        $form.Icon = [System.Drawing.Icon]::new($iconPath)
    } else {
        # Optional: log but don't crash if missing
        Write-Log -Message "app.ico not found at $iconPath (skipping window icon)" -Color 'Gray'
    }
} catch {
